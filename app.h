...
public:
    ...
    CommandPtr getSaveMemCmd();
    CommandPtr getDrawMemCmd();

    void saveMemento( const std::shared_ptr<INode> & frame );
    std::shared_ptr<INode> popMemento();
...
private:
    ...
    std::stack< std::shared_ptr<INode> > m_Mementos;
};